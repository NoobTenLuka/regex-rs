...
impl Regex {
    ...
    pub fn new_with_options(regex: &str, options: RegexOptions) -> Result<Self, RegexCompileError> {
        ...
        let ast = OldParser::parse(&mut token_stream)?;
...
impl OldParser {
    fn parse_rule(
        ...
        match token {
            ...
            Token::EOL => Err(RegexCompileError::EmptyError),
            Token::Dollar => {
                *has_dollar = true;
                Ok(AstNode::Dollar)
            }
